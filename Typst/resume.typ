// modified version of https://github.com/stuxf/basic-typst-resume-template/tree/main/src/resume.typ
...
#let resume(
  ...
  if value != "" {
    let display-value = if link-type != "" {
      link(link-type + value)[#(prefix + value)]
    } else {
      value
    }
...
// Personal Info - Update this section
pad(
  top: 0.25em,
  align(personal-info-position)[
    #{
      let items = (
        contact-item(pronouns, label: "Pronouns"),
        ...
        contact-item(personal-site, link-type: "https://", label: "Website"),
        contact-item(orcid, prefix: [#orcid-icon(color: rgb("#AECD54"))orcid.org/], link-type: "https://orcid.org/", label: "ORCID"),
      )
      items.filter(x => x != none).join("  |  ")
    }
  ],
...
#let edu(
  ...
  if consistent {
    ...
    generic-two-by-two(
      ...
      top-right: dates,
      bottom-left: degree,
      bottom-right: strong(location),
    )
  } else {
    // original edu style (location top-right, dates bottom-right)
    generic-two-by-two(
      top-left: strong(institution),
      top-right: strong(location),
      bottom-left: degree,
      bottom-right: dates,
    )
  }
...
#let workproject(
  ...
  pad(top: -0.5em, bottom: -0.5em, left: 1em)[
    •
    #generic-one-by-two(
      left: {
        [*#name* #if url != "" and dates != "" [ (#link("https://" + url)[#url])]]
